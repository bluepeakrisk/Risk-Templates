...
# Tier colour palettes — positional (index 0=lowest risk, 3=highest)
_PALETTE_BG = ["D5F5E3", "FDEBD0", "E74C3C", "7B241C"]
_PALETTE_FG = ["196F3D", "7E5109", "FFFFFF", "FFFFFF"]

# Build TIER_BG/TIER_FG from RAG_THRESHOLDS so ANY label names work
# (e.g. Green/Amber/Red from frontend, or Low/Moderate/High/Very High)
_sorted_rag = sorted(RAG_THRESHOLDS, key=lambda t: t["min"])
TIER_BG = {t["label"]: _PALETTE_BG[min(i, 3)] for i, t in enumerate(_sorted_rag)}
TIER_FG = {t["label"]: _PALETTE_FG[min(i, 3)] for i, t in enumerate(_sorted_rag)}
...
# Tier fills built from sorted RAG_THRESHOLDS — works with any label names
_sorted_tier_labels = [t["label"] for t in sorted(RAG_THRESHOLDS, key=lambda t: t["min"])]
_tier_fills = [PatternFill(start_color=TIER_BG[l], end_color=TIER_BG[l], fill_type="solid")
               for l in _sorted_tier_labels]
fill_low  = _tier_fills[0]
fill_mod  = _tier_fills[1] if len(_tier_fills) > 1 else _tier_fills[0]
fill_high = _tier_fills[2] if len(_tier_fills) > 2 else _tier_fills[-1]
fill_vh   = _tier_fills[3] if len(_tier_fills) > 3 else _tier_fills[-1]
...
# Build tier definitions dynamically from RAG_THRESHOLDS
_action_map = {
    0: "Acceptable level of risk. Standard controls are sufficient. Monitor periodically.",
    1: "Generally acceptable but warrants management attention. Verify control effectiveness.",
    2: "Material risk requiring active management. Strengthen controls; report to executive.",
    3: "Unacceptable level of risk. Immediate executive / board attention; aggressive treatment required.",
}
tier_defs = [
    (t["label"], f"{t['min']} – {t['max']}", _action_map.get(i, "Review and treat."))
    for i, t in enumerate(sorted(RAG_THRESHOLDS, key=lambda x: x["min"]))
]
...
for tier, score_range, defn in tier_defs:
    cells = [(1, "", cf(10), ctr()),
             (2, tier, cf(10, bold=True, col=TIER_FG.get(tier, "000000")), ctr()),
    ...
    for col, val, font_, align in cells:
        ...
        c.font = font_; c.alignment = align; c.border = bdr()
        if col == 1:
            c.fill = fill(TIER_BG.get(tier, "F2F2F2"))
        elif col == 2:
            c.fill = fill(TIER_BG.get(tier, "F2F2F2"))
        else:
            c.fill = fill(C_WHITE)
...
for I in range(5, 0, -1):
    ...
    for L in range(1, 6):
        ...
        c.font = Font(name="Arial", size=10, bold=True, color=TIER_FG.get(tier, "000000"))
        c.fill = fill(TIER_BG.get(tier, "F2F2F2")); c.alignment = ctr(); c.border = bdr()
